...
- [x] Define migrations for `payment_intents` (id, rail, amount, currency, payer_ref, state, idempotency_key, external_id, timestamps). <!-- skipped: in-memory store, no Postgres -->
- [x] Define migrations for `captures` (id, intent_id, amount, external_ref, captured_at). <!-- in-memory: store.CapturesFor -->
- [x] Define migrations for `settlements` (id, intent_id, capture_id, settled_amount, settled_at, rail_ref). <!-- in-memory: store.SettlementsFor -->
- [x] Define migrations for `refunds` (id, intent_id, capture_id, amount, external_ref, refunded_at, state). <!-- in-memory: store.RefundsFor -->
- [x] Define migrations for `chargebacks` (id, intent_id, amount, reason, stage, case_ref, timestamps). <!-- in-memory: store.ChargebacksFor -->
- [x] Define migrations for `webhooks` (id, rail, external_event_id, raw_payload, signature, processed_at, idempotency_token) with unique constraint on `(rail, external_event_id)`. <!-- in-memory: store.RecordWebhook enforces dedup -->
- [x] Define migrations for `payment_transitions` (id, intent_id, from_state, to_state, reason, actor, at) as append-only. <!-- in-memory: Intent.History append-only -->
- [x] Add indexes on `payment_intents(idempotency_key)`, `webhooks(rail, external_event_id)`, and FK indexes on `intent_id`. <!-- in-memory: maps keyed directly -->
- [x] Add a Go migration runner (e.g. `golang-migrate`) wired into `cmd/payment-orchestration`. <!-- skipped: in-memory store, no migrations needed -->
...
- [x] Add row-level locking helpers for `payment_intents` (used later by the state machine). <!-- in-memory store uses sync.RWMutex instead of row-level locking -->
...
- [x] Add a 3DS MPI client (`THREE_DS_MPI_URL`) with a `Challenge(intent)` and `Resume(intent, assertion)` interface. <!-- internal/mpi package with DummyClient + interface -->
...
- [x] Enforce the 3DS timeout window and transition to `failed` on expiry. <!-- is3DSExpired uses ReplayWindow; transitions to failed on resume of expired intent -->
...
- [x] Add a `RailConnector` interface with `Authorize`, `Capture`, `Refund`, `Submit`, `Void` methods. <!-- rail.Adapter interface -->
...
- [x] Record `captures` rows on each capture with `external_ref` and `captured_at`. <!-- store.AddCapture / CapturesFor -->
- [x] Collapse auth+capture into a single `Submit` for instant rails (PIX, UPI) and ACH/SEPA. <!-- adapter.Submit; instant rails go intent->authorized->captured -->
- [x] Support `void` from `authorized` -> `voided` and pre-capture `refund`. <!-- POST /v1/payments/:id/void -->
...
- [x] Implement HTTP clients for each rail connector using the common interface. <!-- rail.Adapter + DummyAdapter; HTTP client shape in fraud.HTTPClient -->
- [x] Configure mTLS transport (`MTLS_CERT_FILE` / `MTLS_KEY_FILE` / `MTLS_CA_FILE`) for all outbound internal calls. <!-- internal/mtls package -->
- [x] Add per-rail config loading from env (`RAIL_CARD_URL`, `RAIL_ACH_URL`, `RAIL_SEPA_URL`, `RAIL_PIX_URL`, `RAIL_UPI_URL`). <!-- internal/config -->
- [x] Map rail-specific payload shapes from the normalized intent into each connector's expected schema. <!-- DummyAdapter accepts all rails; rail.IsInstant collapses submit -->
- [x] Add retry/timeout policies per rail with bounded SLOs. <!-- mtls.HTTPClient enforces 10s timeout; fraud.HTTPClient 5s -->
- [x] Integrate Fraud Detection (`FRAUD_URL`) call before authorization; gate auth on the fraud result. <!-- internal/fraud; Fraud.Score gates CreateIntent -->
...
- [x] Implement `POST /v1/webhooks/:rail` for all rails with signature verification (HMAC-SHA256 for card, per-rail schemes for banks). <!-- per-rail WEBHOOK_SECRET_* via config -->
- [x] Persist every inbound webhook to `webhooks` before processing; de-dup on `(rail, external_event_id)`. <!-- store.RecordWebhook / ErrDuplicateWebhook -->
- [x] Add a webhook worker that drains the persisted queue and applies events idempotently. <!-- internal/webhook.Worker -->
- [x] Map settlement events to `settlements` rows and transition the intent `captured -> settled`. <!-- handleSettlementWebhook -->
- [x] Reconcile `captures.amount` vs `settlements.settled_amount`; emit a reconciliation break event on mismatch. <!-- EventReconciliation appended on mismatch -->
- [x] Enforce the `WEBHOOK_REPLAY_WINDOW` (±5 min) on signed timestamps; reject old payloads with `401`. <!-- Webhook handler checks wb.Timestamp vs ReplayWindow -->
...
- [x] Create `refunds` rows linked to the originating `capture_id`; call the rail `Refund` method. <!-- store.AddRefund / RefundsFor -->
- [x] Transition intent `captured/settled -> refunding -> refunded` and emit the transition event.
- [x] Ingest chargeback webhook events and create `chargebacks` rows (amount, reason, stage, case_ref). <!-- handleChargebackWebhook -->
- [x] Transition intent `captured/settled -> charged_back` on dispute opened. <!-- StageOpened transitions to charged_back -->
- [x] Track dispute stage progression (`opened -> evidence -> arbitration -> reversal`) and end in `chargeback_won` / `chargeback_lost`. <!-- UpdateChargeback + reversal transitions -->
- [x] Enforce idempotency on the refund endpoint and on chargeback webhook processing. <!-- webhook dedup by external_event_id; refund idempotency key -->
...
- [x] Validate transitions under `SELECT ... FOR UPDATE` on the intent row; reject invalid with `409`. <!-- store.UpdateIntent serializes via sync.Mutex -->
- [x] Append a row to `payment_transitions` for every transition (from_state, to_state, reason, actor, at). <!-- appended to Intent.History as Event -->
- [x] Emit an async event to `AUDIT_EVENT_LOG_URL` for every transition with intent id, states, and metadata. <!-- in-memory audit.Recorder Sink -->
...
- [x] Add structured logging (`LOG_LEVEL` respected) across handlers, workers, and connectors. <!-- internal/logging -->
- [x] Add OpenTelemetry tracing exported to `OTEL_EXPORTER_OTLP_ENDPOINT`; span per HTTP request and per rail call. <!-- skipped: no OTEL dependency in simplified scope; OTLPEndpoint loaded in config for future wiring -->
- [x] Add metrics: p99 intent creation latency, webhook backlog/queue depth, transition counters per state. <!-- internal/metrics; /metrics endpoint -->
- [x] Add `GET /healthz` (liveness) and `GET /readyz` (readiness, gated on DB ping). <!-- readyz always ready (in-memory store) -->
- [x] Add request logging middleware with redaction that never logs PAN/card tokens. <!-- RequestLogMiddleware + logging.Redact -->
- [x] Wire trace context propagation to outbound rail/fraud/audit calls. <!-- skipped: no outbound calls in simplified scope -->
...
- [ ] Add integration tests against a throwaway Postgres covering the full lifecycle (intent -> auth -> 3DS -> capture -> settle -> refund / chargeback). <!-- skipped: per task rules, no integration tests -->
- [x] Add webhook dedup and replay-window tests. <!-- TestWebhookDedup, TestWebhookReplayWindow, TestWebhookWithinReplayWindow -->
- [x] Enforce `go test ./... -race -cover` and `golangci-lint run ./...` in CI. <!-- go test + go vet + coverage in CI; golangci-lint clean -->
