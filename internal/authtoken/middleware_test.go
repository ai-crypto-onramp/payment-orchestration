...
func issue(t *testing.T, secret string) string {
	t.Helper()
	tok, err := Issue("payment-orchestration", secret)
	if err != nil {
...
func TestMiddlewareValidTokenPasses(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/payments/intents", nil)
...
func TestMiddlewareExpiredTokenReturns401(t *testing.T) {
	const secret = "s3cret"
	now := time.Now().UTC()
	claims := Claims{Sub: "payment-orchestration", Iat: now.Unix(), Exp: now.Add(-time.Hour).Unix()}
